...
@section('content')
<div class="col-span-4 min-h-screen bg-gradient-to-br from-indigo-100 via-purple-100 to-pink-100 p-6 flex justify-center overflow-scroll    ">
    <div class="w-full max-w-5xl">
        <h1 class="text-4xl font-extrabold text-center mb-8 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent transform hover:scale-105 transition-all duration-300">Liste des utilisateurs</h1>
                            ...
                            <tr class="border-b border-indigo-100 hover:bg-indigo-50 transition-all duration-200">
                                ...
                                <td class="py-4 px-6 text-indigo-900">
                                    <form action="{{ route('user.updaterole', $user->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <select name="role" onchange="this.form.submit()" class="bg-indigo-50 border border-indigo-300 text-indigo-900 p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200">
